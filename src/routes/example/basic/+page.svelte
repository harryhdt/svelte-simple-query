...
<div>
	...
	{:else if users.data}
		<div>
			<button type="button" onclick={addUser} disabled={state.loadingAdd}
				>{state.loadingAdd ? 'Adding...' : 'Add user'}</button
			>
			<button type="button" onclick={editFirstUser} disabled={state.loadingEdit}
				>{state.loadingEdit ? 'Updating...' : 'Edit 1st user'}</button
			>
			<button
				type="button"
				onclick={() => {
